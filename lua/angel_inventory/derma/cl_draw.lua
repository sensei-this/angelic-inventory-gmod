...
local lifetime = 3;
local local_version = 901.1100; // Draw release from date 
_angel_sensei.versions = _angel_sensei.versions or {};
_angel_sensei.versions[local_version] = true;


local function Initialize(check)
	if (_angel_sensei.versions["_init"] and local_version > _angel_sensei.versions["_latest"] and !check) then 
		_angel_sensei:Alert("[cl_draw] finded new version "..local_version.."");
	else
		if (_angel_sensei.versions["_init"] and _angel_sensei.versions["_latest"] != local_version) then return; end;
		if (check) then _angel_sensei:Alert("[cl_draw] you are using version "..local_version..""); end;
	end;

	function draw.colorAlpha(color, alpha)
		return Color(color.r, color.g, color.b, alpha); // I know that gmod have ColorAlpha function
	end;
	...
	if (check) then
		_angel_sensei.versions["_init"] = true;
	end;
end;
...
timer.Simple(25, function()
	if (_angel_sensei.versions["_init"]) then return; end;

	local latest = 0;
	for k, v in pairs(_angel_sensei.versions) do
	...
	_angel_sensei.versions["_latest"] = latest;
	print("=========================");
	print(local_version);
	PrintTable(_angel_sensei.versions)
	print("=========================");
